...
app.get('/status', async (req,res) => {
  ...
  const hours = uptime_minutes();
  const free_mb = free_disk_mb();
  const record = `${ts}: uptime ${hours.toFixed(2)} minutes, free disk in root: ${free_mb} MBytes`;
